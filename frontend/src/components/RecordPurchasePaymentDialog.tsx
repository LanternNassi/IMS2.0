...
export function RecordPurchasePaymentDialog({ payable, onPaymentRecorded, trigger }: RecordPurchasePaymentDialogProps) {
  ...
  const validateAmount = (value: string) => {
    ...
    if (amount > (payable.grandTotal - payable.paidAmount)) {
      return `Amount cannot exceed outstanding balance of Shs ${(payable.grandTotal - payable.paidAmount).toLocaleString()}`
    }
    return null
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const paymentData = {
        ...
        description: formData.notes || `Payment to ${payable.supplier.companyName}`,
        debtType: "Payable",
        linkedFinancialAccountId: formData.linkedFinancialAccountId || null,
      }

      // Call the API to record payment
      await api.post('/PurchaseDebtsTracker', paymentData)
  ...
  const remainingAfterPayment = formData.amount
    ? (payable.grandTotal - payable.paidAmount) - parseFloat(formData.amount)
    : payable.grandTotal - payable.paidAmount
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={(open) => {
      ...
      <DialogTrigger asChild>
        {trigger || (
          <Button variant="default" size="sm" className="dark:bg-gray-900">
      ...
      <DialogContent className="dark:bg-gray-800 bg-white max-w-lg" transparent>
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Record Payment to Supplier</DialogTitle>
          <DialogDescription>
            Record a payment for {payable.supplier.companyName}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div className="p-4 rounded-lg dark:bg-gray-700/30 bg-gray-50 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="dark:text-gray-400 text-gray-600">Total Purchase Amount:</span>
              <span className="font-semibold dark:text-white text-gray-900">{formatCurrency(payable.grandTotal)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="dark:text-gray-400 text-gray-600">Already Paid:</span>
              <span className="font-semibold text-emerald-500">{formatCurrency(payable.paidAmount)}</span>
            </div>
            <div className="flex justify-between text-sm border-t dark:border-gray-600 border-gray-300 pt-2">
              ...
              <span className="font-bold text-red-500">{formatCurrency(payable.grandTotal - payable.paidAmount)}</span>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t dark:border-gray-700">
            ...
            >
              {isSubmitting ? "Recording..." : `Record ${formData.amount ? formatCurrency(parseFloat(formData.amount)) : "Payment"}`}
            </Button>
          </div>
        </form>
      </DialogContent>
